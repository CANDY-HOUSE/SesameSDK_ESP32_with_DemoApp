...
#if !asm_mult || (uECC_SQUARE_FUNC && !asm_square) || (uECC_SUPPORTS_secp256k1 && (uECC_OPTIMIZATION_LEVEL > 0) && ((uECC_WORD_SIZE == 1) || (uECC_WORD_SIZE == 8)))
...
uECC_VLI_API void uECC_vli_modSquare(uECC_word_t * result, const uECC_word_t * left, const uECC_word_t * mod, wordcount_t num_words)
{
    uECC_word_t product[2 * uECC_MAX_WORDS];
...
int uECC_shared_secret(const uint8_t * public_key, const uint8_t * private_key, uint8_t * secret, uECC_Curve curve)
{
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) _private, private_key, num_bytes);
    __bocpy((uint8_t *) _public, public_key, num_bytes * 2);
#else
    uECC_vli_bytesToNative(_private, private_key, BITS_TO_BYTES(curve->num_n_bits));
    uECC_vli_bytesToNative(_public, public_key, num_bytes);
    uECC_vli_bytesToNative(_public + num_words, public_key + num_bytes, num_bytes);
    ...
    EccPoint_mult(_public, _public, p2[!carry], initial_Z, curve->num_n_bits + 1, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) secret, (uint8_t *) _public, num_bytes);
#else
    uECC_vli_nativeToBytes(secret, num_bytes, _public);
#endif
    return !EccPoint_isZero(_public, curve);
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy(public_key, compressed + 1, curve->num_bytes);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) native, bits, bits_size);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) tmp, private_key, BITS_TO_BYTES(curve->num_n_bits));
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) signature + curve->num_bytes, (uint8_t *) s, curve->num_bytes);
#else
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    __bocpy((uint8_t *) r, signature, curve->num_bytes);
    __bocpy((uint8_t *) s, signature + curve->num_bytes, curve->num_bytes);
#else
    uECC_vli_bytesToNative(_public, public_key, curve->num_bytes);
